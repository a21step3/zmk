...
static void zmk_rgb_underglow_effect_status(void) {
    struct zmk_led_hsb hsb = state.color;
    hsb.b = 0;
...
// ------- Turn on the caps word for status led -------
    ...
    caps_word_hsb.h = zmk_caps_word_state(const struct device *dev) * CONFIG_ZMK_RGB_UNDERGLOW_STATUS_CAPS_COLOR;
    caps_word_hsb.b = zmk_caps_word_state(const struct device *dev) * caps_word_hsb.b;
